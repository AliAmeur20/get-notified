...
import ChannelReq from "../components/channel-req-a";
...
function AdminChReq () {
    ...
    useEffect(()=>{
        ...
        const fetchChannels = async ()=>{
          const response = await fetch(`/api/channels?valide=false&page=${page}&search=${search}&location=${location}&category=${category}`,{
            headers :{
        ...
        <div className="container-fluid my-5 px-4">
            ...
            <div className="row">
              {channels && channels.map((channel)=>(
                  <ChannelReq key={channel._id} {...channel} />
              ))}
              ...
              {((total===0) || (channels && Object.keys(channels).length===0)) && <div className="text-center">
                ...
                <h3 className="text-primary mt-3">there's no results..</h3>
                </div>}
