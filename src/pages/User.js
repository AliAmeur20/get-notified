...
function User () {
    ...
    const {channels , dispatch : gChannels} = useChannelContext()
    ...
    useEffect(()=>{
      const fetchProfile =async () => {
        ...
        const fetchChannels = async ()=>{
          ...
          if(response.ok){
            gChannels({type:'GET_CHANNELS', payload:json.channels})
            setTotal(json.total)
            setIsLoading(false)
          }
          if(!response.ok){
        ...
        if(user){
          ...
          },[user , dispatch , search , gChannels , category , location , page])
        ...
        <div className="container-fluid my-5 px-4">
          ...
          <div className="row">
            {channels && channels.map((channel)=>(
                <ChannelU key={channel._id} {...channel} />
            ))}
